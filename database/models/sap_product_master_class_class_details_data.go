...
var SapProductMasterClassClassDetailsDatumWhere = struct {
	...
	KeyDate:                  whereHelpernull_String{field: "`sap_product_master_class_class_details_data`.`KeyDate`"},
...
var (
	sapProductMasterClassClassDetailsDatumAllColumns            = []string{"ClassInternalID", "KeyDate", "ClassType", "ClassTypeName", "Class", "ClassStatus", "ClassStatusName", "ClassGroup", "ClassGroupName", "CreationDate", "LastChangeDate", "ValidityStartDate", "ValidityEndDate", "ClassLastChangedDateTime"}
	sapProductMasterClassClassDetailsDatumColumnsWithoutDefault = []string{"ClassInternalID", "KeyDate", "ClassType", "ClassTypeName", "Class", "ClassStatus", "ClassStatusName", "ClassGroup", "ClassGroupName", "CreationDate", "LastChangeDate", "ValidityStartDate", "ValidityEndDate", "ClassLastChangedDateTime"}
	sapProductMasterClassClassDetailsDatumColumnsWithDefault    = []string{}
	sapProductMasterClassClassDetailsDatumPrimaryKeyColumns     = []string{"ClassInternalID"}
...
func SapProductMasterClassClassDetailsData(mods ...qm.QueryMod) sapProductMasterClassClassDetailsDatumQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`sap_product_master_class_class_details_data`.*"})
	}

	return sapProductMasterClassClassDetailsDatumQuery{q}
}

// FindSapProductMasterClassClassDetailsDatum retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindSapProductMasterClassClassDetailsDatum(ctx context.Context, exec boil.ContextExecutor, classInternalID string, selectCols ...string) (*SapProductMasterClassClassDetailsDatum, error) {
	...
	query := fmt.Sprintf(
		"select %s from `sap_product_master_class_class_details_data` where `ClassInternalID`=?", sel,
	)
...
func (o *SapProductMasterClassClassDetailsDatum) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `sap_product_master_class_class_details_data` WHERE `ClassInternalID`=?"
...
func (o *SapProductMasterClassClassDetailsDatum) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindSapProductMasterClassClassDetailsDatum(ctx, exec, o.ClassInternalID)
	if err != nil {
		return err
	}
...
func SapProductMasterClassClassDetailsDatumExists(ctx context.Context, exec boil.ContextExecutor, classInternalID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `sap_product_master_class_class_details_data` where `ClassInternalID`=? limit 1)"
