...
func (r *sapPurchaseOrderItemAccountDatumR) GetPurchaseOrderSapPurchaseOrderHeaderDataWithAddress() *SapPurchaseOrderHeaderDataWithAddress {
	if r == nil {
		return nil
	}
...
var (
	...
	sapPurchaseOrderItemAccountDatumGeneratedColumns      = []string{}
)
...
func SapPurchaseOrderItemAccountData(mods ...qm.QueryMod) sapPurchaseOrderItemAccountDatumQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`sap_purchase_order_item_account_data`.*"})
	}

	return sapPurchaseOrderItemAccountDatumQuery{q}
}
