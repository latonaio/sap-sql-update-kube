...
func (r *sapPurchaseRequisitionItemDeliveryAddressDatumR) GetPurchaseRequisitionSapPurchaseRequisitionHeaderDatum() *SapPurchaseRequisitionHeaderDatum {
	if r == nil {
		return nil
	}
...
func SapPurchaseRequisitionItemDeliveryAddressData(mods ...qm.QueryMod) sapPurchaseRequisitionItemDeliveryAddressDatumQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`sap_purchase_requisition_item_delivery_address_data`.*"})
	}

	return sapPurchaseRequisitionItemDeliveryAddressDatumQuery{q}
}
