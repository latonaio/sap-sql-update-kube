...
var SapProductMasterClassProductClassDatumRels = struct {
	...
	ProductSapProductMasterClassClassDetailsData    string
}{
	ProductSapProductMasterClassProductGeneralDatum: "ProductSapProductMasterClassProductGeneralDatum",
	ProductSapProductMasterClassClassDetailsData:    "ProductSapProductMasterClassClassDetailsData",
}
...
type sapProductMasterClassProductClassDatumR struct {
	...
	ProductSapProductMasterClassClassDetailsData    SapProductMasterClassClassDetailsDatumSlice `boil:"ProductSapProductMasterClassClassDetailsData" json:"ProductSapProductMasterClassClassDetailsData" toml:"ProductSapProductMasterClassClassDetailsData" yaml:"ProductSapProductMasterClassClassDetailsData"`
}
...
// ProductSapProductMasterClassClassDetailsData retrieves all the sap_product_master_class_class_details_datum's SapProductMasterClassClassDetailsData with an executor via Product column.
func (o *SapProductMasterClassProductClassDatum) ProductSapProductMasterClassClassDetailsData(mods ...qm.QueryMod) sapProductMasterClassClassDetailsDatumQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`sap_product_master_class_class_details_data`.`Product`=?", o.Product),
	)

	query := SapProductMasterClassClassDetailsData(queryMods...)
	queries.SetFrom(query.Query, "`sap_product_master_class_class_details_data`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`sap_product_master_class_class_details_data`.*"})
	}
...
// LoadProductSapProductMasterClassClassDetailsData allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (sapProductMasterClassProductClassDatumL) LoadProductSapProductMasterClassClassDetailsData(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSapProductMasterClassProductClassDatum interface{}, mods queries.Applicator) error {
	var slice []*SapProductMasterClassProductClassDatum
	var object *SapProductMasterClassProductClassDatum
	...
	query := NewQuery(
		qm.From(`sap_product_master_class_class_details_data`),
		qm.WhereIn(`sap_product_master_class_class_details_data.Product in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load sap_product_master_class_class_details_data")
	}

	var resultSlice []*SapProductMasterClassClassDetailsDatum
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice sap_product_master_class_class_details_data")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on sap_product_master_class_class_details_data")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for sap_product_master_class_class_details_data")
	}
	...
	if singular {
		object.R.ProductSapProductMasterClassClassDetailsData = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &sapProductMasterClassClassDetailsDatumR{}
			}
			foreign.R.ProductSapProductMasterClassProductClassDatum = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.Product, foreign.Product) {
				local.R.ProductSapProductMasterClassClassDetailsData = append(local.R.ProductSapProductMasterClassClassDetailsData, foreign)
				if foreign.R == nil {
					foreign.R = &sapProductMasterClassClassDetailsDatumR{}
				}
				foreign.R.ProductSapProductMasterClassProductClassDatum = local
				break
			}
		}
	}
...
// AddProductSapProductMasterClassClassDetailsData adds the given related objects to the existing relationships
// of the sap_product_master_class_product_class_datum, optionally inserting them as new records.
// Appends related to o.R.ProductSapProductMasterClassClassDetailsData.
// Sets related.R.ProductSapProductMasterClassProductClassDatum appropriately.
func (o *SapProductMasterClassProductClassDatum) AddProductSapProductMasterClassClassDetailsData(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*SapProductMasterClassClassDetailsDatum) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.Product, rel.ClassInternalID, rel.KeyDate}
	...
	if o.R == nil {
		o.R = &sapProductMasterClassProductClassDatumR{
			ProductSapProductMasterClassClassDetailsData: related,
		}
	} else {
		o.R.ProductSapProductMasterClassClassDetailsData = append(o.R.ProductSapProductMasterClassClassDetailsData, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &sapProductMasterClassClassDetailsDatumR{
				ProductSapProductMasterClassProductClassDatum: o,
			}
		} else {
			rel.R.ProductSapProductMasterClassProductClassDatum = o
		}
	}
	return nil
}

// SetProductSapProductMasterClassClassDetailsData removes all previously related items of the
// sap_product_master_class_product_class_datum replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.ProductSapProductMasterClassProductClassDatum's ProductSapProductMasterClassClassDetailsData accordingly.
// Replaces o.R.ProductSapProductMasterClassClassDetailsData with related.
// Sets related.R.ProductSapProductMasterClassProductClassDatum's ProductSapProductMasterClassClassDetailsData accordingly.
func (o *SapProductMasterClassProductClassDatum) SetProductSapProductMasterClassClassDetailsData(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*SapProductMasterClassClassDetailsDatum) error {
	query := "update `sap_product_master_class_class_details_data` set `Product` = null where `Product` = ?"
	...
	if o.R != nil {
		for _, rel := range o.R.ProductSapProductMasterClassClassDetailsData {
			queries.SetScanner(&rel.Product, nil)
			if rel.R == nil {
				continue
			}

			rel.R.ProductSapProductMasterClassProductClassDatum = nil
		}

		o.R.ProductSapProductMasterClassClassDetailsData = nil
	}
	return o.AddProductSapProductMasterClassClassDetailsData(ctx, exec, insert, related...)
}

// RemoveProductSapProductMasterClassClassDetailsData relationships from objects passed in.
// Removes related items from R.ProductSapProductMasterClassClassDetailsData (uses pointer comparison, removal does not keep order)
// Sets related.R.ProductSapProductMasterClassProductClassDatum.
func (o *SapProductMasterClassProductClassDatum) RemoveProductSapProductMasterClassClassDetailsData(ctx context.Context, exec boil.ContextExecutor, related ...*SapProductMasterClassClassDetailsDatum) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("Product")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.ProductSapProductMasterClassClassDetailsData {
			if rel != ri {
				continue
			}

			ln := len(o.R.ProductSapProductMasterClassClassDetailsData)
			if ln > 1 && i < ln-1 {
				o.R.ProductSapProductMasterClassClassDetailsData[i] = o.R.ProductSapProductMasterClassClassDetailsData[ln-1]
			}
			o.R.ProductSapProductMasterClassClassDetailsData = o.R.ProductSapProductMasterClassClassDetailsData[:ln-1]
			break
		}
	}
...
func SapProductMasterClassProductClassData(mods ...qm.QueryMod) sapProductMasterClassProductClassDatumQuery {
	...
	return sapProductMasterClassProductClassDatumQuery{NewQuery(mods...)}
}
